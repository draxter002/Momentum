    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-2 md:p-4">
      <div className="bg-gray-900 rounded-xl shadow-2xl w-full h-full max-w-full max-h-full overflow-hidden border border-gray-700">
        <div className="p-4 md:p-6 border-b border-gray-700 flex items-center justify-between">
          <div>
            <h2 className="text-xl md:text-2xl font-bold text-white mb-1">Gold Streak Milestones</h2>
        ...
        <div className="overflow-y-auto p-4 md:p-6" style={{ maxHeight: 'calc(100vh - 150px)' }}>
          {['early', 'intermediate', 'advanced', 'legendary'].map((tier) => {
            ...
            return (
              <div key={tier} className="mb-6 md:mb-8 last:mb-0">
                <div className="flex items-center gap-2 mb-3 md:mb-4">
                  <h3 className={`text-lg md:text-xl font-bold ${TIER_COLORS[tier]?.text || 'text-gray-400'}`}>
                    {getTierTitle(tier)}
                  </h3>
                    ...
                    >
                      ...
                      {milestone.achieved && milestone.count > 1 && (
                        <div className="absolute -top-2 -left-2">
                          <div className="bg-amber-500 rounded-full px-1.5 md:px-2 py-0.5 md:py-1 shadow-lg border-2 border-amber-300">
                            <span className="text-[10px] md:text-xs font-bold text-white">×{milestone.count}</span>
                          </div>
                        </div>
                      )}

                      <div className="text-center">
                        <div className={`text-3xl md:text-5xl mb-1 md:mb-2 ${milestone.achieved ? 'animate-pulse-slow' : ''}`}>
                          {milestone.emoji}
                        </div>
                        <h4 className={`text-sm md:text-base font-semibold mb-1 ${milestone.achieved ? 'text-white' : 'text-gray-500'}`}>
                          {milestone.name}
                        </h4>
                        <p className={`text-xs md:text-sm ${milestone.achieved ? 'text-gray-300' : 'text-gray-600'}`}>
        ...
        <div className="p-3 md:p-4 border-t border-gray-700 bg-gray-850 text-center">
